...
async function seedIfNeeded() {
  ...
  async function insertDeliverables(sectionId, items) {
    await supabase.from('deliverables').insert(
      items.map((item) => ({ section_id: sectionId, ...item }))
    );
  }
  ...
  for (const name of ['SBS', 'SBS 3', 'TSS 1']) {
    ...
    await insertDeliverables(sectionId, [
      ...commonDeliverables.map((title, i) => ({
        number: i + 1, title, status: 'No', delivery_date: null, comments: null, is_doc_type: false,
      })),
      ...docTypes.map((title) => ({
        number: null, title, status: 'No', delivery_date: null, comments: null, is_doc_type: true,
      })),
    ]);
  }
  ...
  // LCCH 3
  {
    const sectionId = await insertSection('LCCH 3');
    await insertDeliverables(sectionId, [
  ...
  // LCCH 4
  {
    const sectionId = await insertSection('LCCH 4');
    await insertDeliverables(sectionId, [
      { number: 0, title: 'ESG Compliance Roadmap', status: 'Ongoing', delivery_date: '22/05/26', comments: null, is_doc_type: false },
