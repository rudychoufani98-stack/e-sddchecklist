...
function requireAdminOrConstruction(req, res, next) {
  // Consultants (incl. category field consultants) get read access — the Site Map
  // and the field-capture screen need the project list + progress.
  const role = req.user?.role || '';
  if (['admin', 'construction', 'consultant'].includes(role) || role.startsWith('consultant_')) return next();
