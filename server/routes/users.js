...
const ROLES = ['admin', 'viewer', 'submitter'];

// GET all users (no password hashes)
router.get('/', async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('users').select('username, role').order('username');
...
router.post('/', async (req, res) => {
  try {
    ...
    const { error } = await supabase.from('users').insert({ username, password_hash, role });
    if (error) throw error;
