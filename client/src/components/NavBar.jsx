...
// Chat nav tab with an unread badge (full chat lives on the /chat page)
function ChatNavLink({ user }) {
  ...
  useEffect(() => {
    ...
    const check = () => api.get('/chat').then(r => {
      ...
      if (location.pathname === '/chat') {
        const newest = r.data.length ? new Date(r.data[r.data.length - 1].created_at).getTime() : 0;
        localStorage.setItem('chatLastSeen', String(newest));
        setUnread(0);
      } else {
        const seen = Number(localStorage.getItem('chatLastSeen') || 0);
        setUnread(r.data.filter(m => new Date(m.created_at).getTime() > seen && m.username !== user?.username).length);
      }
    }).catch(() => {});
    check();
    const t = setInterval(check, location.pathname === '/chat' ? 4000 : 20000);
  ...
  return (
    <Link to="/chat"
      className={`relative px-3.5 py-2 rounded-lg text-sm font-medium transition-all duration-150 ${
        active ? 'bg-white/15 text-white shadow-inner ring-1 ring-white/10' : 'text-blue-100/90 hover:bg-white/10 hover:text-white'
      }`}>
      Chat
      {unread > 0 && (
        <span className="absolute -top-1 -right-1 min-w-[16px] h-4 px-1 bg-[#e63946] text-white text-[10px] font-black rounded-full flex items-center justify-center ring-2 ring-[#1a3c5e]">
          {unread > 99 ? '99+' : unread}
        </span>
      )}
    </Link>
